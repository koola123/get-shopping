...
export const CartItem = ({ item }) => {
   const { addToCart, removeFromCart, increaseAmount, decreaseAmount } = useContext(CartContext);
      ...
      <img className="max-w-[80px]" src={image} alt="" />
        ...
        <div className="flex gap-x-2 h-[36px] text-sm ">
          ...
          h-full border text-primary font-medium">
           ...
           <div 
           className="flex-1 flex justify-end items-center text-primary font-medium">{`${parseFloat(price * amount).toFixed(2)}`}
           </div>
        </div>
        </div>
      </div>
